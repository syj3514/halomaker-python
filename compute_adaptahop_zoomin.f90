...
!=======================================================================
   ...
   memory_used = memory_used + real(size(liste_parts, kind=8), kind=8)*4.d0/1.d9
   ...
   memory_used = memory_used + real(size(pos, kind=8), kind=8)*8.d0/1.d9
   pos(:,:) = pos_in(:,:)
   allocate(mass(npart))
   memory_used = memory_used + real(size(mass, kind=8), kind=8)*8.d0/1.d9
   mass(:) = mass_in(:)
   if (megaverbose) write(errunit,*) "[Memory] Start create_tree_structure"

   allocate(refmask(npart))
   memory_used = memory_used + real(size(refmask, kind=8), kind=8)*1/1.d9
   refmask(:) = refmask_in(:)
   ...
   if (megaverbose) write(errunit,*) "[Memory] Start dev_compute_saddle_list"
   call print_memory()
...
subroutine node2table
   ...
   memory_used = memory_used + real(size(real_table, kind=8), kind=8)*8.d0/1.d9
   allocate(integer_table(6,nnodes)) ! mass, level, mother, firstchild, nsisters, sister
   memory_used = memory_used + real(size(integer_table, kind=8), kind=8)*4.d0/1.d9
   do inodee=1,nnodes
   ...
   memory_used = memory_used - real(size(node, kind=8)*int((4*6)+(8*8), kind=8), kind=8)/1.d9
   deallocate(node)
...
!=======================================================================
   ...
   memory_used = memory_used + real(size(density, kind=8), kind=8)*8.d0/1.d9
   nactive=0
   call print_memory()
   ...
   !$OMP END DO
   !$OMP END PARALLEL
   ! if(verbose) write(errunit,*) "Test A"
   ! if(verbose) write(errunit,*) "Test A"
   ! if(verbose) write(errunit,*) "Test A"
   ! if(verbose) write(errunit,*) "Test A"
   ! if(verbose) write(errunit,*) "Test A"
...
!=======================================================================
   ...
   memory_used = memory_used + real(size(idpart_adapt, kind=8), kind=8)*4.d0/1.d9
   call print_memory()
   ...
   memory_used = memory_used + real(size(densityg, kind=8), kind=8)*8.d0/1.d9
   allocate(firstpart(ngroups))
   memory_used = memory_used + real(size(firstpart, kind=8), kind=8)*4.d0/1.d9
   call print_memory()
...
!=======================================================================
   ...
   memory_used = memory_used + real(size(idgroup, kind=8), kind=8)*4.d0/1.d9
   allocate(color(ngroups))
   memory_used = memory_used + real(size(color, kind=8), kind=8)*4.d0/1.d9
   allocate(igroupid(ngroups))
   memory_used = memory_used + real(size(igroupid, kind=8), kind=8)*4.d0/1.d9
   allocate(idgroup_tmp(ngroups))
   memory_used = memory_used + real(size(idgroup_tmp, kind=8), kind=8)*4.d0/1.d9
   ...
   memory_used = memory_used + real(size(queue_color, kind=8), kind=8)*4.d0/1.d9
...
&                                ' of each elementary group...'
   ...
   memory_used = memory_used + real(size(dev_group_nhnei, kind=8), kind=8)*4.d0/1.d9
   allocate(dev_group_nhnei(ngroups))
   memory_used = memory_used + real(size(dev_group_offset, kind=8), kind=8)*4.d0/1.d9
   allocate(dev_group_offset(ngroups+1))
   memory_used = memory_used + real(size(dev_isad_all, kind=8), kind=8)*4.d0/1.d9
   allocate(touch_thr(ngroups,nbPes))
   memory_used = memory_used + real(size(touch_thr, kind=8), kind=8)*4.d0/1.d9
   allocate(listg_thr(ngroups,nbPes))
   memory_used = memory_used + real(size(listg_thr, kind=8), kind=8)*4.d0/1.d9
...
!=======================================================================
   ...
   memory_used = memory_used + real(size(dev_rho_saddle_all, kind=8), kind=8)*8.d0/1.d9
   allocate(dev_rho_saddle_all(max(total_conn,1)))

   if (max_neig.gt.0) then
      memory_used = memory_used + real(size(rho_sad, kind=8), kind=8)*8.d0/1.d9
      memory_used = memory_used + real(size(isad, kind=8), kind=8)*4.d0/1.d9
      memory_used = memory_used + real(size(indx, kind=8), kind=8)*4.d0/1.d9
      allocate(rho_sad(max_neig))
      allocate(isad(max_neig))
      allocate(indx(max_neig))
      allocate(rho_sad_thr(max_neig,nbPes))
      memory_used = memory_used + real(size(rho_sad_thr, kind=8), kind=8)*8.d0/1.d9
   else
      allocate(rho_sad(1))
      allocate(isad(1))
      allocate(indx(1))
...
!  Count remaining connections per group
   ...
   memory_used = memory_used + real(size(new_group_nhnei, kind=8), kind=8)*4.d0/1.d9
   allocate(new_group_offset(ngroups+1))
   memory_used = memory_used + real(size(new_group_offset, kind=8), kind=8)*4.d0/1.d9
...
!  New offsets
   ...
   memory_used = memory_used + real(size(new_isad_all, kind=8), kind=8)*4.d0/1.d9
   allocate(new_rho_saddle_all(max(new_total_conn,1)))
   memory_used = memory_used + real(size(new_rho_saddle_all, kind=8), kind=8)*8.d0/1.d9
...
!  Replace old CSR arrays with compacted CSR arrays
   ...
   deallocate(isad)
   deallocate(indx)
   memory_used = memory_used - real(size(mass_cell, kind=8), kind=8)*4.d0/1.d9
   memory_used = memory_used - real(size(size_cell, kind=8), kind=8)*8.d0/1.d9
   memory_used = memory_used - real(size(pos_cell, kind=8), kind=8)*8.d0/1.d9
   memory_used = memory_used - real(size(sister, kind=8), kind=8)*4.d0/1.d9
   memory_used = memory_used - real(size(firstchild, kind=8), kind=8)*4.d0/1.d9
   memory_used = memory_used - real(size(idpart, kind=8), kind=8)*4.d0/1.d9
...
!=======================================================================
   ...
   memory_used = memory_used + real(size(idpart_tmp, kind=8), kind=8)*4.d0/1.d9
   allocate(mass_cell(ncellmx))
   memory_used = memory_used + real(size(mass_cell, kind=8), kind=8)*8.d0/1.d9
   ...
   memory_used = memory_used + real(size(icidpart_tmp, kind=8), kind=8)*4.d0/1.d9
   call print_memory()
   call system_clock(count=ttt0, count_rate=tttrate)
...
!=======================================================================
 ...
 &          'ncellmx is too small. Increase it and reallocate arrays accordingly'
         ...
         memory_used = memory_used + real(size(mass_cell_tmp, kind=8), kind=8)*8.d0/1.d9
         ...
         memory_used = memory_used + real(size(sister_tmp, kind=8), kind=8)*4.d0/1.d9
         ...
         memory_used = memory_used + real(size(firstchild_tmp, kind=8), kind=8)*4.d0/1.d9
         ...
         memory_used = memory_used + real(size(size_cell_tmp, kind=8), kind=8)*8.d0/1.d9
         ...
         memory_used = memory_used + real(size(pos_cell_tmp, kind=8), kind=8)*8.d0/1.d9
